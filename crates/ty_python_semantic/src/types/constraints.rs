...
impl<'db, 'c> ConstraintSet<'db, 'c> {
    ...
    pub(crate) fn iff(
        ...
        self.verify_builder(builder);
        Self::from_node(builder, self.node.iff(builder, other.node))
    }
...
impl<'db> ConstraintSetBuilder<'db> {
    ...
    pub(crate) fn load<'c>(
        ...
        ConstraintSet::from_node(self, remap_node(&nodes, other.node))
    }
...
impl InteriorNode {
    ...
    fn abstract_one_inner<'db>(
        ...
        if should_remove(self_interior.constraint) {
            ...
            Node::new_constraint(builder, self_interior.constraint).ite(builder, if_true, if_false)
        }
    }
    ...
    fn solutions<'db, 'c>(
        ...
        fn solutions_inner<'db, 'c>(
            ...
            // The paths in the BDD, and the constraints in each path, will both come out in a
            // stable order, which is based on the order that the underlying typevars and
            // constraints were added to the builder. That should ensure that we construct
            // any unions or intersections in our type mappings in a stable order.
            let mut paths = Vec::new();
            interior.for_each_path(db, builder, |path| {
                let path: Vec<_> = path.positive_constraints().collect();
                paths.push(path);
            });
...
mod tests {
    ...
    fn test_display_graph_output() {
        ...
        // construction more interesting. Note that the BDD variable ordering lines up with the
